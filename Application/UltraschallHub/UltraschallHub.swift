...
class UltraschallHub: NSViewController, NSTableViewDataSource, NSTableViewDelegate {
    ...
    func setDriverLoaded(loaded: Bool!) {
        let connection = NSXPCConnection(serviceName: "fm.ultraschall.UltraschallHubXPCService")
        connection.remoteObjectInterface = NSXPCInterface(withProtocol: UltraschallHubXPCServiceProtocol.self)
        connection.resume()
        
        let tester = connection.remoteObjectProxyWithErrorHandler {
            (error) in NSLog("remote proxy error: %@", error)
        } as! UltraschallHubXPCServiceProtocol
        
        tester.upperCaseString("test") { (data) -> Void in
            NSLog(data);
        }
